...
public class PlaceLerper : MonoBehaviour
{
    ...
    private void Awake()
    {
        foreach (Transform place in placeList)
        {
    ...
    public void Lerp(bool set = false)
    {
        ...
        if (toLerpRotation && !set) transform.rotation = Quaternion.Lerp(transform.rotation, targetRotation, Time.deltaTime * positionlerpSpeed);
